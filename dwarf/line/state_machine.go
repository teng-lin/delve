...
// the traced program.
func (dbl *DebugLineInfo) NextLocation(pc uint64) *Location {
	var (
...
func (dbl *DebugLineInfo) LocationInfoForFileLine(f string, l int) *Location {
	var (
		sm  = newStateMachine(dbl)
		buf = bytes.NewBuffer(dbl.Instructions)
	)

	executeUntilFileAndLine(sm, buf, f, l)
